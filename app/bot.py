...
def _refresh_gdrive_token_if_needed():
    """Refresh GDrive access token using refresh_token if current one is expired."""
    try:
        cfg = load_persistent_config()
        drive_auth = cfg.get("drive_auth_token")
        if not isinstance(drive_auth, dict):
            return
        refresh_token = str(drive_auth.get("refresh_token", "")).strip()
        ...
        if r.status_code != 200:
            logger.warning("[GDRIVE] Token refresh failed: %s", r.text[:200])
            return
        token_data = r.json()
        new_access = token_data.get("access_token", "")
        if not new_access:
            return
        drive_auth["access_token"] = new_access
        drive_auth["expires_in"] = token_data.get("expires_in", 3599)
        update_persistent_config(drive_auth_token=drive_auth)
        _write_rclone_drive_token(drive_auth)
        logger.info("[GDRIVE] Token refreshed successfully")
    except Exception as e:
        logger.warning("[GDRIVE] Token refresh error: %s", e)
...
def gdrive_upload(local_path):
    ...
    if not os.path.exists(RCLONE_CONF):
        return False, "", "rclone config not found"
    _refresh_gdrive_token_if_needed()
...
async def run_pipeline(context, chat_id, video_link, face_link=None, job_mode="direct", progress_seed_message_id=None, queue_job_id=None, gender_mode_override=None, stage_event_hook=None):
    ...
    try:
        ...
        if target.name != original_target_name:
            _out_stem = re.sub(r"[^A-Za-z0-9._-]+", "_", Path(original_target_name).stem).strip("._-") or "output"
            if re.match(r"^mega_[A-Za-z0-9]{6,12}$", _out_stem):
                _out_stem = "video"
            _out_stem = _out_stem[:40].strip("._-") or "output"
